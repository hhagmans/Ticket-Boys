...
public class DailyChecker implements Serializable
{
	...
	public static void trigger(ShiftDao shiftDao)
	{
		...
		try
		{
			...
			log.info("ShiftReminder: Email send to : " + latestEmployee.getEmail());
			log.info("ShiftReminder: Email send to : " + soonestEmployee.getEmail());
		}
		catch (Exception e1)
		{
			log.error(e1);
	...
	public static void conflictTrigger(ConflictDao conflictDao)
	{
		...
		log.debug("ConflictReminder: Found " + findAll.size() + " conflicts. Now checking for unsolved ones");
		...
		for (Conflict conflict : findAll)
		{
			...
			if (startDateofConflict.after(currentTime) && startDateofConflict.before(inTenDays))
			{
				employees.add(conflict.getEmployee());
			}
		}

		log.debug("ConflictReminder: Going to send " + employees.size() + " emails.");
		...
		for (Employee employee : employees)
		{
			...
			try
			{
				log.info("ConflictReminder: Sending Email to: " + employee.getEmail());
				EmailUtil.sendEmailWithSubject(subject, msg, employee.getEmail());
			}
			catch (Exception e)
			{
				log.error(e);
